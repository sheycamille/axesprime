...
use App\Model\Plans;
use App\Model\CpTransaction;
use App\Model\TpTransaction;
use App\Model\User_plans;
...
trait BCPTrait
{
    ...
    public function paywithcp($amount, $coin, $up, $ui, $msg)
    {
        ...
        if ($msg == 'new') {
            ...
            $user = User::where('tele_id', $ui)->first();
                ...
                if ($result['error'] == 'ok') {
                    ...
                    if ($status >= 100 || $status == 2) {
                        ...
                        if (!empty($user->ref_by) || $user->ref_by != "0") {
                            //fetch site settings
                            $bot_link = Setting::getValue('bot_link');
                            $referral_commission = Setting::getValue('referral_commission');
                            //start to Process referral
                            $ref_by_link = $bot_link . "?start=$user->ref_by";
                            //get the referral
                            $duser = User::where('ref_link', $ref_by_link)->first();
                            ...
                            //Add ref earning to the User account balance
