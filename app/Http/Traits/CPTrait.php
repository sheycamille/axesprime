...
trait CPTrait
{
    ...
    {
        //fetch settings
        $eth_address = Setting::getValue('eth_address');
        $btc_address = Setting::getValue('btc_address');
        $ltc_address = Setting::getValue('ltc_address');
        ...
        if ($msg == 'new') {
            ...
            $cpd = CpTransaction::where('id', '1')->first();
            ...
            if ($result['error'] == 'ok') {
                ...
                return ('<div style="border:1px solid #f2f2f2; padding:10px; margin:150px; color:#d0d0d0; text-align:center;"><h1>You will be redirected to your payment page!</h1><h4 style="color:#222;">Click on the button below to proceed.</h4><form action="' . $result['result']['status_url'] . '" method="post"><input type="hidden" name="_token" value="' . csrf_token() . '"><inputtype="submit" class="btn btn-default" name="submit" value="Proceed"><a href="' . route('deposits') . '" class="btn btn-default">Cancel</a><form></div');
    ...
    public function cpaywithcp()
    {
        ...
        $cp_txns = CpTransaction::where('status', NULL)->get();
        if (count($cp_txns) > 0) {
                ...
                if ($result['error'] == 'ok') {
                    ...
                    if ($status >= 100 || $status == 2) {
                        ...
                        if (!empty($user->ref_by) || $user->ref_by != "0") {
                            ...
                            $earnings = $referral_commission * $txn->amount_paid / 100;
    ...
    public function cpwithdraw($amount, $coin, $wallet, $ui, $to_withdraw)
    {
        ...
        $cpd = CpTransaction::where('id', '1')->first();
        //get settings
        $s_currency = Setting::getValue('s_currency');
        $this->Setup($cpd->cp_pv_key, $cpd->cp_p_key);

        $amount = $amount;
        $add_tx_fee = 1;
        $currency = $coin;
        $currency2 = $s_currency;
        $address = $wallet;
    ...
    public function creditLeadershipBonus($sponsor, $amnt)
    {
        ...
        foreach ($array as $entry) {
            ...
            if ($level == 1) {
                ...
                $earnings = 1.5 * $amnt / 100;
            } elseif ($level == 3) {
                $earnings = 1 * $amnt / 100;
            } elseif ($level == 4) {
                $earnings = 0.75 * $amnt / 100;
            } elseif ($level == 5) {
                $earnings = 0.5 * $amnt / 100;
            } elseif ($level == 6) {
                $earnings = 0.4 * $amnt / 100;
            } elseif ($level == 7) {
                $earnings = 0.3 * $amnt / 100;
            } elseif ($level == 8) {
                $earnings = 0.25 * $amnt / 100;
            } elseif ($level == 9) {
                $earnings = 0.20 * $amnt / 100;
            } elseif ($level == 10) {
                $earnings = 0.10 * $amnt / 100;
            }
