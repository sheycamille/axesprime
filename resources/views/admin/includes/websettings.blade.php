<form method="post" action="{{ route('updatepreference') }}" enctype="multipart/form-data">
    <div class="form-group">
        ...
        <input type="text"
            ...
            name="contact_email" value="{{ \App\Models\Setting::getValue('contact_email') }}" required>
    </div>

    <div class="sign-up1">
        <h3 class="text-{{ $text }}"> Deposit Email:</h3>
    </div>
    <div class="form-group">
        <input type="text"
            class="form-control bg-{{ Auth('admin')->User()->dashboard_style }} text-{{ $text }}"
            name="deposit_email" value="{{ \App\Models\Setting::getValue('deposit_email') }}" required>
    </div> <br>

    <div class="sign-up1">
        <h3 class="text-{{ $text }}"> Withdrawal Email:</h3>
    </div>
    <div class="form-group">
        <input type="text"
            class="form-control bg-{{ Auth('admin')->User()->dashboard_style }} text-{{ $text }}"
            name="withdrawal_email" value="{{ \App\Models\Setting::getValue('withdrawal_email') }}" required>
    </div> <br>

    <div class="sign-up1">
        <h3 class="text-{{ $text }}"> Verification email:</h3>
    </div>
    <div class="form-group">
        <input type="text"
            class="form-control bg-{{ Auth('admin')->User()->dashboard_style }} text-{{ $text }}"
            name="verification_email" value="{{ \App\Models\Setting::getValue('verification_email') }}" required>
    </div> <br>


    <div class="form-group">
        <h5 class="text-{{ $text }}">Uploaded Files Location</h5>
        <small class="text-{{ $text }}">Note: To use AWS S3, please supply your AWS information in the .ENV
            file</small>
    ...
    <div class="form-group">
        <div class="sign-u">
            ...
            <div class="sign-up2">
                <small class="text-{{ $text }}">if turned on, Users will have the option of changing their
