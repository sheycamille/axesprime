<form method="post" action="{{route('updatepreference')}}" enctype="multipart/form-data">
    <div class="form-group">
        ...
        <input type="text" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" name="contact_email" value="{{\App\Models\Setting::getValue('contact_email')}}" required>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="currency" id="select_c" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" onchange="s_f()">
            <option value="<?php echo htmlentities(\App\Models\Setting::getValue('currency')); ?>">{{ \App\Models\Setting::getValue('currency') }}</option>
